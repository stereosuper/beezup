...
module.exports = function(wrapper, windowWidth, tempo) {
    ...
    let animTl = [],
        animRunning = [];
    const animSvg = wrapper.find('.js-animSvg');

    let easeOut = Power3.easeOut,
        easeIn = Power3.easeIn;
    const bounce = CustomEase.create(
        'custom',
        'M0,0 C0.4,0 0.593,0.983 0.6,1 0.662,0.916 0.664,0.88 0.7,0.88 0.742,0.88 0.8,0.985 0.814,0.998 0.825,0.994 1,1 1,1'
    );
    const revBounce = CustomEase.create(
        'custom',
        'M0,0 C0,0 0.061,-0.004 0.095,-0.015 0.178,-0.043 0.229,-0.074 0.315,-0.104 0.353,-0.118 0.38,-0.124 0.42,-0.13 0.441,-0.133 0.458,-0.132 0.48,-0.129 0.498,-0.126 0.513,-0.124 0.53,-0.115 0.566,-0.095 0.596,-0.078 0.625,-0.048 0.667,-0.004 0.694,0.035 0.725,0.091 0.767,0.169 0.788,0.223 0.82,0.309 0.86,0.421 0.879,0.487 0.91,0.604 0.949,0.757 1,1 1,1'
    );
    ...
    function animChoose(svg) {
        ...
        const t1 = svg.find('#cable-3-1');
        const t2 = svg.find('#cable-3-2');
        const t4 = svg.find('#cable-3-4');

        const c1 = svg.find('#connector-3-1');
        const c2 = svg.find('#connector-3-2');
        const c3 = svg.find('#connector-3-3');
        const c4 = svg.find('#connector-3-4');

        TweenLite.set([c1, c2, c3, c4], { y: -50, opacity: 0 });
        TweenLite.set([t1, t2, t4], { drawSVG: 0 });

        tlBox
            .to(c1, tempo, { opacity: 1, ease: easeIn })
            ...
            .staggerTo(
                [c1, c2, c3, c4],
                tempo,
                { delay: tempo * 2, opacity: 0, y: -50, ease: easeOut },
                0.1
            );
    ...
    function animImport(svg) {
        ...
        let checks = svg
            ...
            .toArray()
            .reverse();
        const containerBlocks = svg.find('#blocks');
        const cable = svg.find('#cable-4');
        const tlCheck = new TimelineLite({ paused: true, delay: tempo * 2 });
        let yMove = -50,
            xMove = 66,
            lastBloc,
            lastElt,
        ...
        function addLastTweens() {
            tweenCheck1 = TweenLite.to(currentCheck, tempo, {
                opacity: 1,
                delay: -tempo * 2,
                ease: easeIn,
            });
            tweenCheck2 = TweenLite.to(currentCheck, tempo, {
                y: 0,
                ease: bounce,
                delay: -tempo * 2,
                onComplete: loopMove,
        ...
        TweenLite.set(cable, { drawSVG: 0 });
        TweenLite.set([checks[3], checks[2], checks[1], checks[0]], {
            y: -50,
            opacity: 0,
        });
    ...
    function animHistory(svg) {
        ...
        const h1 = svg.find('#hour-5-1');
        const h2 = svg.find('#hour-5-2');
        const h3 = svg.find('#hour-5-3');
        const h4 = svg.find('#hour-5-4');
        const b1 = svg.find('#connector-5-1');
        const b2 = svg.find('#connector-5-2');
        const b3 = svg.find('#connector-5-3');
        const b4 = svg.find('#connector-5-4');
        const t2 = svg.find('#txt-5-2');
        var tl = new TimelineLite({
            paused: true,
            onComplete() {
                tl.restart();
        ...
        tl.to(h1, tempo, { opacity: 1, ease: easeIn, delay: tempo * 2 })
            ...
            .add([
                TweenLite.to(t2, tempo, {
                    delay: -tempo,
                    ease: easeIn,
                    fill: '#1D1D1B',
                }),
                TweenLite.to(h2, tempo, {
                    delay: -tempo,
                    ease: easeIn,
                    fill: '#DE0C20',
                }),
            ...
            .add([
                TweenLite.to([h1, h2, h3, h4], tempo * 2, {
                    opacity: 0,
                    ease: easeOut,
                    delay: -tempo,
                }),
                TweenLite.to(t2, tempo * 2, {
                    fill: '#0096E0',
                    ease: easeOut,
                    delay: -tempo,
                }),
    ...
    function animStats(svg) {
        ...
        const blocks = svg.find('.bloc-6');
        const shadows = svg.find('.shade-6').toArray();

        blocks.each((i, el) => {
            subTl[i] = new TimelineLite();

            subTl[i]
                .to(el, 2, { y: -10, delay: i * 0.2 })
    ...
    function animOptimize(svg) {
        ...
        const t1 = svg.find('#cable-7-1');
        const t3 = svg.find('#cable-7-3');
        const t4 = svg.find('#cable-7-4');
        const p1 = svg.find('#percent-7-1');
        const p2 = svg.find('#percent-7-2');
        const p3 = svg.find('#percent-7-3');
        const p4 = svg.find('#percent-7-4');
        const b1 = svg.find('#connector-7-1');
        const b2 = svg.find('#connector-7-2');
        const b3 = svg.find('#connector-7-3');
        const b4 = svg.find('#connector-7-4');
        const s2 = svg.find('#symbol-7-2');
        const s4 = svg.find('#symbol-7-4');
        var tl = new TimelineLite({
            paused: true,
            onComplete() {
                tl.restart();
        ...
        tl.to(p1, tempo, { opacity: 1, ease: easeIn, delay: tempo * 2 })
            ...
            .to(b2, tempo, { opacity: 1, y: 0, delay: tempo, ease: bounce })
            .add([
                TweenLite.to(s2, tempo, {
                    delay: -tempo,
                    ease: easeIn,
                    fill: '#BD0314',
                }),
                TweenLite.to(p2, tempo, {
                    delay: -tempo,
                    ease: easeIn,
                    fill: '#DE0C20',
                }),
            ...
            .to(b4, tempo, { opacity: 1, y: 0, delay: tempo, ease: bounce })
            .add([
                TweenLite.to(s4, tempo, {
                    delay: -tempo,
                    ease: easeIn,
                    fill: '#24DA4B',
                }),
                TweenLite.to(p4, tempo, {
                    delay: -tempo,
                    ease: easeIn,
                    fill: '#24DA4B',
                }),
            ...
            .add([
                TweenLite.to([p1, p2, p3, p4], tempo * 2, {
                    opacity: 0,
                    ease: easeOut,
                    delay: -tempo,
                }),
                TweenLite.to([s2, s4], tempo * 2, {
                    fill: '#0096E0',
                    ease: easeOut,
                    delay: -tempo,
                }),
    ...
    function animOrders(svg) {
        ...
        const boxes = svg.find('#boxes-8');
        const tl = new TimelineLite({ paused: true });
        let xMove = 48,
            yMove = 36;
        let loop = 0,
            newElt;
        const t1 = svg.find('#cable-8-1');
        const t2 = svg.find('#cable-8-2');
        const t3 = svg.find('#cable-8-3');
        ...
        function resetLast(last, row) {
            ...
            TweenLite.fromTo(
                ...
                tempo,
                { y: '-=50', opacity: 0 },
                { y: '+=50', opacity: 1, ease: bounce, delay: tempo * 5 }
            );
        }

        function loopMove() {
            const boxes = svg.find('.box');
            let box1 = svg.find('#box-1'),
                box2 = svg.find('#box-2'),
                box3 = svg.find('#box-3');
            ...
            if (loop === 1) {
                tl.to(boxes, tempo, {
                    ...
                    opacity: 0,
                    ease: revBounce,
                    onCompleteParams: [box3, 3],
                    onComplete: resetLast,
                });
            } else if (loop === 3) {
                tl.to(boxes, tempo, {
                    x: `+=${xMove}`,
    ...
    function animStock(svg) {
        ...
        const t1 = svg.find('#cable-9-1');
        const t2 = svg.find('#cable-9-2');
        const t3 = svg.find('#cable-9-3');
        const t4 = svg.find('#cable-9-4');

        const gameboyCount = svg.find('#gameboyCount');
        const keyboardCount = svg.find('#keyboardCount');
        const joystickCount = svg.find('#joystickCount');

        const bees1 = svg.find('#bees-count-1');
        const bees2 = svg.find('#bees-count-2');
        const bees3 = svg.find('#bees-count-3');
        const beesTl1 = animBees(bees1, tempo, true, tempo * 2);
        const beesTl1SmallDelay = animBees(bees1, tempo, true, tempo);
        const beesTl2 = animBees(bees2, tempo, true, tempo * 2);
        const beesTl2SmallDelay = animBees(bees2, tempo, true, tempo);
        const beesTl3 = animBees(bees3, tempo, true, tempo * 2);
        const beesTl3SmallDelay = animBees(bees3, tempo, true, tempo);
            ...
            .add([
                ...
                TweenLite.to(t2, tempo, {
                    ...
                    onComplete() {
                        ...
                        synchCounter(
                            keyboardCount,
                            svg.find('.js-keyboard').length
                        );
                        setTimeout(() => {
                            // remove a joystick in counter
                            joystickCount.html(
                                parseInt(joystickCount.html()) - 1
                            );
                        }, tempo * 5000);
                    },
                }),
            ...
            .add([
                ...
                TweenLite.to(t2, tempo, {
                    ...
                    onComplete() {
                        ...
                        synchCounter(
                            gameboyCount,
                            svg.find('.js-gameboy').length
                        );
                        setTimeout(() => {
                            // remove a keyboard in counter
                            keyboardCount.html(
                                parseInt(keyboardCount.html()) - 1
                            );
                        }, tempo * 5000);
                    },
                }),
            ...
            .add([
                ...
                TweenLite.to(t2, tempo, {
                    ...
                    onComplete() {
                        ...
                        synchCounter(
                            joystickCount,
                            svg.find('.js-joystick').length
                        );
                        setTimeout(() => {
                            // remove a gameboy in counter
                            gameboyCount.html(
                                parseInt(gameboyCount.html()) - 1
                            );
                        }, tempo * 5000);
                    },
                }),
    ...
    function animModules(svg) {
        ...
        const t1 = svg.find('#cable-10-1');
        const t2 = svg.find('#cable-10-2');
        const t3 = svg.find('#cable-10-3');
        var tl = new TimelineLite({
            paused: true,
            onComplete() {
                tl.restart();
    ...
    function scrollHandler() {
        ...
        animSvg.each(function(i) {
            if (
                scrollTop + windowHeight - $(this).data('height') >
                    $(this).data('offsetTop') &&
                scrollTop < $(this).data('offsetTop') + $(this).data('height')
            ) {
                if (!animRunning[i]) {
                    animTl[i].play();
                    animRunning[i] = true;
                }
            } else if (animRunning[i]) {
                animTl[i].pause();
                animRunning[i] = false;
            }
        });
